...
INDICATOR: dict = { # color
    'ON': 'limegreen',
    'OFF': 'grey' # lightgrey | lightyellow
}
...
class CTkSlideView(CTkFrame):
    def __init__(self, *args,
        ...
        **kwargs
        ...
        self.left_anim = CTkAnimator(from_=0, to=self.w, acceleration=0.02) # ACCELERATOR
        self.right_anim = CTkAnimator(from_=0, to=-self.w, acceleration=0.02)
        self.indicator_frame = CTkFrame(self, height=18, corner_radius=0, fg_color="transparent") 
        ...
        if self.left_anim.completed != True:
            ...
            for i, x in enumerate(self.tabs):
                ...
                if self.tabs[i][1] == self.w/2:
                    self.tabs[i][2].configure(fg_color=INDICATOR['ON'])
                else:
                    self.tabs[i][2].configure(fg_color=INDICATOR['OFF'])
    ...
    def create_tab(self):
        ...
        lbl = CTkLabel(self.indicator_frame,
            width=8,
            height=8,
            fg_color=INDICATOR['OFF'],
            corner_radius=8,
            text="",
            font=("SF Display", 5)
        )
        ...
        self.tabs[0][2].configure(fg_color=INDICATOR['ON']) "white"
        self.left_btn.lift()
        self.right_btn.lift()
        self.guide += self.w
        return frame
